        ...
        <div className="overflow-x-auto bg-transparent md:bg-white md:rounded-lg md:shadow-none">
          <Table className="block md:table">
            ...
            <TableBody className="block md:table-row-group">
              {filteredExpenses.map(expense => <TableRow key={expense.id} className="mobile-card-row">
                <TableCell data-label="الوصف">{expense.description}</TableCell>
                <TableCell data-label="المبلغ">{formatCurrency(expense.amount)}</TableCell>
                <TableCell data-label="الفئة">{getCategoryDisplayName(expense.category || "general")}</TableCell>
                <TableCell data-label="طريقة الدفع">{getPaymentMethodDisplayName(expense.paymentMethod || "cash")}</TableCell>
                <TableCell data-label="الإجراءات">
                  <div className="flex gap-2 justify-end md:justify-start">
                    <Button variant="outline" size="sm" onClick={() => setEditingExpense(expense)} className="h-10 w-10">
                      <Edit size={16} />
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => setDeletingExpense(expense)} className="h-10 w-10 text-red-500 hover:text-red-700">
                      <Trash size={16} />
                    </Button>
                  </div>
                </TableCell>
              </TableRow>)}
              {filteredExpenses.length === 0 && <TableRow className="block md:table-row bg-white rounded-lg shadow-sm border border-gray-100 p-4 md:p-0 md:border-0 md:shadow-none md:rounded-none">
                <TableCell colSpan={5} className="text-center text-gray-500 block md:table-cell">
